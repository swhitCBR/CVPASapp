...
HOR_mod_wrap <- function(HOR_mod_ls,
  ...
  sel_rows_tmp2 <- sel_rows_tmp1 |> dplyr::rename(log.VNS.ent.hor=VNS,
                                                  barrier.exit.hor.fac=barrierTF,
                                                  CVP.exit.hor=CVP,
                                                  SWP.exit.hor=SWP)
  ...
  # simp_top_mod <- HOR_mod_ls[["HOR_d2_mods"]][[1]]
  # pred_ls <- predict(object=simp_top_mod,newdata = sel_rows_tmp4,se.fit = T)
  # return(pred_ls)

  # HOR_mod_ls$HOR_d2_aictab[names(HOR_mod_ls$HOR_TCJ_d2_mods),]
  # aic_avg_tb_wts <- aic_avg_tb |> dplyr::mutate(AICwt=exp(-0.5*dAIC)/sum(exp(-0.5*aic_avg_tb$dAIC)))
  aic_avg_tb <- HOR_mod_ls$HOR_d2_aictab
  aic_avg_tb_wts <- aic_avg_tb |> dplyr::mutate(AICwt=exp(-0.5*dAIC)/sum(exp(-0.5*aic_avg_tb$dAIC)))
  
  tmp_HOR_preds1 <- get_glmmTMB_ests(sel_data_in = sel_rows_tmp4,
                                             mods_obj_nm='HOR_d2_mods',
                                             aic_avg_tb_wts_in=aic_avg_tb_wts,
                                             glmmTMB_res_ls_in=HOR_mod_ls,
                                            sub_estimate_in="HOR")
  ...
  # return(tmp_HOR_preds1)
  # return(tmp_HOR_preds2)
  ...
  # aic_avg_tb <- HOR_mod_ls$HOR_TCJ_aictab[names(HOR_mod_ls$HOR_TCJ_d2_mods),]
  # # print(aic_avg_tb)
  # aic_avg_tb_wts <- aic_avg_tb |> dplyr::mutate(AICwt=exp(-0.5*dAIC)/sum(exp(-0.5*aic_avg_tb$dAIC)))
