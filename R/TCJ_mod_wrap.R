...
TCJ_mod_wrap <- function(TCJ_mod_ls,
  ...
  # simp_top_mod <- TCJ_mod_ls[["HOR_d2_mods"]][[1]]
  # pred_ls <- predict(object=simp_top_mod,newdata = sel_rows_tmp4,se.fit = T)
  # return(pred_ls)
  
  # TCJ_mod_ls$HOR_d2_aictab[names(TCJ_mod_ls$HOR_TCJ_d2_mods),]
  ...
  # aic_avg_tb <- TCJ_mod_ls$HOR_TCJ_aictab[names(TCJ_mod_ls$HOR_TCJ_d2_mods),]
  # # print(aic_avg_tb)
  # aic_avg_tb_wts <- aic_avg_tb |> dplyr::mutate(AICwt=exp(-0.5*dAIC)/sum(exp(-0.5*aic_avg_tb$dAIC)))
