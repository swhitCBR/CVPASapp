...
for(ii in 1:nrow(AIC_DF_d2)){
  ...
  predDF_comb <- data.frame(predDF,pred_DF_boot_summ,xpred_tmp)
  ...
  library(ggplot2)
  ggplot() +
    geom_ribbon(data=predDF_comb #%>% filter(!(id %in% c(3,5,6)))
                ,aes(y=plogis(EST),x=DOY,ymin=plogis(LCLadj),ymax=plogis(UCLadj)),
                alpha=0.25,fill="red",color=NA) +# geom_point() +
    geom_line(data=predDF_comb #%>% filter(!(id %in% c(3,5,6)))
              ,aes(y=plogis(EST),x=DOY)) +
    geom_ribbon(data=predDF_comb #%>% filter(!(id %in% c(3,5,6)))
                ,aes(y=plogis(EST),x=DOY,ymin=plogis(LCL),ymax=plogis(UCL)),
                alpha=0.5,color=NA) +# geom_point() +
    geom_line(data=predDF_comb #%>% filter(!(id %in% c(3,5,6)))
              ,aes(y=plogis(EST),x=DOY)) +
    facet_wrap(~Year)
  ...
  ggplot() +
    ...
    geom_line(data=predDF_comb #%>% filter(!(id %in% c(3,5,6)))
              ,aes(y=plogis(EST),x=DOY)) +
    geom_ribbon(data=predDF_comb #%>% filter(!(id %in% c(3,5,6)))
                ,aes(y=plogis(EST),x=DOY,ymin=plogis(boot_lcl),ymax=plogis(boot_ucl)),
                alpha=0.5,color=NA) +# geom_point() +
    geom_line(data=predDF_comb #%>% filter(!(id %in% c(3,5,6)))
              ,aes(y=plogis(boot_mean),color="blue",x=DOY)) +
    facet_wrap(~Year)
  
  # head(pred_DF_boot_summ)# head(pDOY_arvDF_lred_DF_boot_summ)
