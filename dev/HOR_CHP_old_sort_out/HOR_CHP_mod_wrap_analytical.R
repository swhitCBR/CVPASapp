...
for(ii in 1:nrow(AIC_DF_d2)){
  ...
  pt_ests_SE_DF_ls[[ii]] <- data.frame(ii=,subset(HOR_CHP_TMB_all_mods$pt_estsDF,dm==dm_str_val),SE=  sqrt(diag(SD_obj$cov.fixed)))
  # wt_v <- AIC_DF_d2$
}
...
HOR_CHP_pred_comp_ls <- list(
   ...
   # HOR_CHP_mod_dat_sep_ls[c("x.df","Xc.3v8")]
   )

     # "x.df"=HOR_CHP_mod_dat_sep_ls$"x.df",
     # "Xc.3v8"=HOR_CHP_mod_dat_sep_ls$"Xc.3v8",
     # "sdat.det.common2"=HOR_CHP_mod_dat_sep_ls$"sdat.det.common2",
     # "sdat.det"=HOR_CHP_mod_dat_sep_ls$"sdat.det"
     # )
saveRDS(HOR_CHP_pred_comp_ls,"HOR_CHP_pred_comp_ls.rds")
