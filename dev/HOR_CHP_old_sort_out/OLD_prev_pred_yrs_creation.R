...
# tidyr::pivot_longer(overall_DF)



matrix(c(c(1,2,3),c(1,2,3)),nrow=2)^2

# pred_pSE_DF_w1 <- pred_pDF |> dplyr::filter(Year==2011 & DOY %in% 20:24) |> dplyr::select(Year,DOY,param,p_SEadj) |> tidyr::pivot_wider(names_from=param,values_from = p_SEadj)
# pred_pDF  |> dplyr::select(Year,DOY,param,pr_pred) |> tidyr::pivot_wider(names_from=param,values_from = pr_pred) |> relocate(Year,DOY)
# pred_pDF  |> dplyr::select(Year,DOY,param,p_SEadj) |> tidyr::pivot_wider(names_from=param,values_from = p_SEadj)



pred_tab_ls$TCJ$lo_SEadj
pred_tab_ls$HOR$lo_SEadj
pred_tab_ls$HOR_TCJviaSJL$lo_SEadj
pred_tab_ls$HOR_CHPviaSJL$lo_SEadj
pred_tab_ls$HOR_CHPviaORE$lo_SEadj
pred_tab_ls$TCJ_CHPviaMAC$lo_SEadj
pred_tab_ls$TCJ_CHPviaTRN$lo_SEadj



pred_tab_ls$TCJ$lo_pred
pred_tab_ls$HOR$lo_pred
pred_tab_ls$HOR_TCJviaSJL$lo_pred
pred_tab_ls$HOR_CHPviaSJL$lo_pred
pred_tab_ls$HOR_CHPviaORE$lo_pred
pred_tab_ls$TCJ_CHPviaMAC$lo_pred
pred_tab_ls$TCJ_CHPviaTRN$lo_pred
